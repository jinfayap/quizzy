                  ...
                  <modal ref="addPermission">
                    ...
                    <template v-slot:button>
                      <button
                        ...
                        @click="addPermission(role, index)"
                      >
                        Add
                      </button>
                    </template>
                  ...
                  <modal ref="deletePermission">
                    ...
                    <template v-slot:button>
                      <button
                        ...
                        @click="removePermission(role, index)"
                      >
                        Remove
                      </button>
                    </template>
...
export default {
  ...
  methods: {
    addPermission(role, index) {
      axios
        .post(`/role/${role.id}/permission/${this.selectedPermission}`)
        .then((response) => {
          this.$refs.addPermission[index].isOpen = false;
    ...
    removePermission(role, index) {
      axios
        .delete(`/role/${role.id}/permission/${this.selectedPermission}`)
        .then((response) => {
          this.$refs.deletePermission[index].isOpen = false;

          this.roles = this.roles.filter((r) => {
            if (r.id == role.id) {
              r.permissions = r.permissions.filter(
                (p) => p.id != this.selectedPermission
              );
            }
            return r;
          });
